...
export class ModalMedicoComponent implements OnInit {
  ...
  constructor(
    ...
    this._especialidadServicio.listaActivos().subscribe({
      next: (data) => {
        if (data.isExistoso) this.listaEspecialidades = data.resultado;
      },
      error: (e) => {}
    });
  }
  ...
  crearModificarMedico() {
    ...
    if(this.datosMedico == null) {
      this._medicoServicio.crear(medico).subscribe({
        next: (data) => {
          if(data.isExistoso) {
            this._compartidoServicio.mostrarAlerta('Grabado con exito', 'Completo');
      ...
      this._medicoServicio.editar(medico).subscribe({
        next: (data) => {
          if(data.isExistoso) {
            this._compartidoServicio.mostrarAlerta('Actualizado con exito', 'Completo');
            this.modal.close("true");
          }
          else
